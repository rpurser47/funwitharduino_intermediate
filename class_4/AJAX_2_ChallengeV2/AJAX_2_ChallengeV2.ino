...
void indexCmd(WebServer &server, WebServer::ConnectionType type, char *command, bool)
{
  ...
  if (type == WebServer::HEAD)
  {
    ...
    sendStringInSRAM(server,PSTR("function getSensorData() {"));
    ...
    sendStringInSRAM(server,PSTR("var sensordata = JSON.parse(this.responseText);"));
    sendStringInSRAM(server,PSTR("document.getElementById(\"tmp102_txt\").innerHTML = sensordata.Tmp102;"));
    sendStringInSRAM(server,PSTR("document.getElementById(\"hh10d_txt\").innerHTML = sensordata.HH10D;"));
    sendStringInSRAM(server,PSTR("document.getElementById(\"MPL3115A1Pres_txt\").innerHTML = sensordata.MPL3115A1Pres;"));
    sendStringInSRAM(server,PSTR("document.getElementById(\"MPL3115A1Temp_txt\").innerHTML = sensordata.MPL3115A1Temp;"));
    ...
    sendStringInSRAM(server,PSTR("setTimeout('getSensorData()', 2000);"));    
    sendStringInSRAM(server,PSTR("}"));
    sendStringInSRAM(server,PSTR("</script>"));
    sendStringInSRAM(server,PSTR("</head>"));
    sendStringInSRAM(server,PSTR("<body onload=\"getSensorData()\">"));
    sendStringInSRAM(server,PSTR("<h1>Arduino AJAX Sensor Monitor</h1>"));
    sendStringInSRAM(server,PSTR("<p>Tmp102: <span id=\"tmp102_txt\">Unread</span>&degC</p>"));
    sendStringInSRAM(server,PSTR("<p>HH10D: <span id=\"hh10d_txt\">Unread</span>%</p>"));
    sendStringInSRAM(server,PSTR("<p>MPL3115A1 Pressure: <span id=\"MPL3115A1Pres_txt\">Unread</span> Pa</p>"));
    sendStringInSRAM(server,PSTR("<p>MPL3115A1 Temp: <span id=\"MPL3115A1Temp_txt\">Unread</span>&degC</p>"));
    sendStringInSRAM(server,PSTR("</body>"));
    sendStringInSRAM(server,PSTR("</html>"));
}

void getSensorDataCmd(WebServer &server, WebServer::ConnectionType type, char *command, bool)
{
  ...
  //  Respond with a JSON object containing the readings.
  server.print("{\"Tmp102\":");server.print(readTmp102());
  float RH;
  if(readHh10dRH(&RH))
  {
    server.print(",\"HH10D\":");server.print(RH);
  }
  float pressureInPa, altTempInC;
  if(readMPL3115A1(&pressureInPa, &altTempInC))
  {
    server.print(",\"MPL3115A1Pres\":");server.print(pressureInPa);
    server.print(",\"MPL3115A1Temp\":");server.print(altTempInC);
  }
  server.print("}");
}
...
void setup()
{
  ...
  webserver.addCommand("ajax_data", &getSensorDataCmd);
