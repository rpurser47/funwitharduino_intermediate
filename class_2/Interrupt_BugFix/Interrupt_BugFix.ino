...
//   !!BUG!! This interrupt is called thousands of times per second...but millis() is only good to 1ms!
  }
...
//   !!BUG!! These should be longs -- hh10dInterruptCount is an unsigned long, but this compiler does not flag that.  
//           If you increase the time between readings of the hh10d, you should greater accuracy, but you'll get crazy
//           results if any of the values from the interrupt routine exceed 32767.
  ...
  float periodInMs = float(finish - start) / 1000.0;
  // Need to divide by 1000 to convert from microseconds to ms
